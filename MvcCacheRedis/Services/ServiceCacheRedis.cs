...
namespace MvcCacheRedis.Services
{
    ...
    {
        //IMPORTANTE EL OBJETO 'IDatabase' QUE ES LA BASE DE DATOS QUE NOS PERMITE ACCEDER 
        ...
        {
            //CACHE REDIS FUNCIONA CON CLAVES UNICAS QUE DEBERIAN SER 
            //POR USUARIO, NOSOTROS NO TENEMOS LOGIN, TODAS LAS PERSONAS
            //VERAN LOS MISMOS FAVORITOS, LO SUYO SERIA CONCATENAR LA CLAVE CON
            //EL ID DEL USUARIO (En este caso no se hara)
            //ALMACENAREMOS UNA COLECCION DE PROODUCTOS
            //LO PRIMERO SERA VISUALIZAR SI TENEMOS PRODUCTOS ALMACENADOS PREVIAMENTE 
        ...
        public void DeleteProductoFavorito(int idproducto)
        {
            ...
            if(favoritos != null)
            {
                ...
                {
                    //SERIALIZAMOS LA COLECCION SIN EL NUEVO PRODUCTO (YA BORRADO)
                    string jsonproductos = JsonConvert.SerializeObject(favoritos);
                    //ALMACENMAOS DE NUEVO LOS PRODUCTOS
